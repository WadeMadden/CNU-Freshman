...
public class JosephusListTest {
	...
	public void testCopyListOfZeroOrOne() {
		...
		try {
			...
			// final assertion that list is actual copy not just pointers
			// to the same nodes in original 
			assertNotEquals("Copy should have its own nodes", original.first, list.first);
	...
	public void testCopyListOfMany() {
		...
		try {
			...
			// final assertion that list is actualy copy not just pointers
			// to the same nodes in original
			assertNotEquals("Copy should have its own nodes", original.first, list.first);
	...
	public void testJosephusOnSmallList() {
		...
		// list of two
		try {
			// list already contains dragon[0]
			list.add(dragon[1]);
			String result = list.runProblem(3);
	...
	public void testJosephusOnLargeList1() {
		...
		try {
			...
			//should be able to run a second time with the same result
			result = list.runProblem(out);
